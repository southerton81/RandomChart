...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let leaderboardView = LeaderboardView(container)
        
        let contentView = MainTabView(chartView, leaderboardView)
    ...
    func createChartObservableObject() -> ChartObservableObject {
        let fetchRequest = NSFetchRequest<ChartState>(entityName: "ChartState")
        if let chartStates = try? container.context().fetch(fetchRequest) {
            if (!chartStates.isEmpty) {
                let chartState = chartStates[0]
                return ChartObservableObject(chartState.chartLen, chartState.seed)
            }
        }
        return ChartObservableObject()
    }
